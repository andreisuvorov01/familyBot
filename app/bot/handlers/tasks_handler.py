...
async def handle_text_message(message: types.Message, db_user: User, session):
    ...
    # Проверяем режим или наличие префикса
    is_prefix = message.text.lower().startswith(('л ', 'с '))
    if db_user.task_creation_mode == TaskCreationMode.MESSAGE or is_prefix:
        title, visibility, deadline, error = TaskParser.parse_message(message.text)
        ...
        # Если это просто сообщение в режиме команд и без префикса - ничего не делаем или даем подсказку
        pass
